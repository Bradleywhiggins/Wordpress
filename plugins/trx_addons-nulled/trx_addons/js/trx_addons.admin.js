...
(function() {
	...
	jQuery(document).ready(function() {
		...
		jQuery('body')
			.on('change', 'select[class*="post_type"],select[class*="taxonomy"]', function () {
				...
				if (cat_fld.length > 0) {
					...
					trx_addons_refresh_list(cat_fld.attr('class').indexOf('taxonomy') >= 0
												? 'taxonomies'
												: 'terms',
											jQuery(this).val(),
											cat_fld,
		...
		window.trx_addons_layout_selector_refresh_link = function(selector) {
			var parent = selector.data('setting') === 'layout'
							? selector.parents('.elementor-control-content')
							: selector.parent();
