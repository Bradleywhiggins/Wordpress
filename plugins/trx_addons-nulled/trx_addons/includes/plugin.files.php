...
if ( !function_exists( 'trx_addons_enqueue_slider' ) ) {
	function trx_addons_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			// 3.4.2 - Latest version with jQuery support
			if (true) {
			...
			// 4+ - New version with ES6 support (not need jQuery)
			} else {
				wp_enqueue_style(  'swiperslider', trx_addons_get_file_url('js/swiper/new/swiper.min.css'), array(), null );
				wp_enqueue_script( 'swiperslider', trx_addons_get_file_url('js/swiper/new/swiper.min.js'), array(), null, true );
			}
...
if (!function_exists('trx_addons_get_file_dir')) {	
	function trx_addons_get_file_dir($file, $return_url=false) {
		...
		$dir = '';
		if (($theme_dir = apply_filters('trx_addons_filter_get_theme_file_dir', '', TRX_ADDONS_PLUGIN_BASE.'/'.($file), $return_url)) != '')
			$dir = $theme_dir;
...
if (!function_exists('trx_addons_get_folder_dir')) {	
	function trx_addons_get_folder_dir($folder, $return_url=false) {
		...
		$child_url = get_stylesheet_directory_uri().'/'.TRX_ADDONS_PLUGIN_BASE.'/';
		$dir = '';
		if (($theme_dir = apply_filters('trx_addons_filter_get_theme_folder_dir', '', TRX_ADDONS_PLUGIN_BASE.'/'.($folder), $return_url)) != '')
			$dir = $theme_dir;
