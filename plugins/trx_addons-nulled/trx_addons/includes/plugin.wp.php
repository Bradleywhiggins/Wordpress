	...
	function trx_addons_get_post_counters($counters='views', $show=false) {
		...
		foreach ($components as $comp) {
			...
			// Custom counter
				...
				$output = apply_filters('trx_addons_filter_show_post_counter', $output, $comp, $post_id);
...
if (!function_exists('trx_addons_categories_fill_custom_column')) {
	...
	function trx_addons_categories_fill_custom_column($output='', $column_name='', $tax_id=0) {
		if ($column_name == 'category_image' && ($cat_img = trx_addons_get_category_image($tax_id))) {
			?><img class="trx_addons_image_selector_preview trx_addons_category_image_preview" src="<?php echo esc_url(trx_addons_add_thumb_size($cat_img, trx_addons_get_thumb_size('tiny'))); ?>" alt="<?php esc_attr_e("Category thumb", 'trx_addons'); ?>"><?php
		}
		if ($column_name == 'category_icon' && ($cat_img = trx_addons_get_category_icon($tax_id))) {
			?><img class="trx_addons_thumb_selector_preview trx_addons_category_icon_preview" src="<?php echo esc_url(trx_addons_add_thumb_size($cat_img, trx_addons_get_thumb_size('tiny'))); ?>" alt="<?php esc_attr_e("Category icon", 'trx_addons'); ?>"><?php
		}
	}
}
...
if ( !function_exists( 'trx_addons_get_theme_info' ) ) {
	function trx_addons_get_theme_info() {
		static $theme_info = false;
		if ($theme_info === false) {
			$theme = wp_get_theme();
			...
			$theme_info = apply_filters('trx_addons_filters_get_theme_info', array(
																			...
																			'theme_activated' => '',
																			'theme_pro_key' => '',
																			'theme_page_url' => menu_page_url( 'trx_addons_theme_panel', false ),
																			'theme_categories' => '',
																			'theme_plugins' => '',
																			'theme_feed' => array(),
																			'theme_actions' => array(),
