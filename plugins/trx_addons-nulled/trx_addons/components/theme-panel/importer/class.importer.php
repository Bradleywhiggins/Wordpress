...
class trx_addons_demo_data_importer {
		...
		<div id="trx_addons_theme_panel_section_<?php echo esc_attr($tab_id); ?>" class="trx_addons_tabs_section">
			...
			if ( trx_addons_theme_panel_is_theme_activated() ) {
				...
				?>
					...
					<?php $this->build_page($tab_id, $theme_info); ?>
			...
			if ( $this->options['allow_import'] && !isset($_POST['exporter_action']) ) { 
				?><div class="trx_importer_section">
					...
					<h1 class="trx_addons_theme_panel_section_title">
						<?php esc_html_e( 'Demo Data', 'trx_addons' ); ?>
					</h1>
					...
					if (!empty($tab_id)) {
						...
						<div class="trx_addons_theme_panel_section_info">
							<p><?php echo wp_kses_data( __( 'Import demo data partially or do a full demo import.', 'trx_addons' ) ); ?></p>
							<p><?php echo wp_kses_data( __( 'In the first case, demo data will be added to your existing content. In the second case, demo data will replace your existing content.', 'trx_addons' ) ); ?></p>
						</div>
						<?php
					}
					...
					?><form id="trx_importer_form">
						...
						?><p class="trx_importer_description trx_importer_description_both"><?php echo wp_kses_data('<b>Some plugins</b> (for example BuddyPress and bbPress) <b>increases the memory consumption</b> at 60-80MB depending on the configuration. So if you have problems with installing the demo-data due to lack of memory - deactivate some plugins and then retry the installation of the demo-data.', 'trx_addons'); ?></p><?php
	...
	function import_widgets() {
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $k=>$v) {
				update_option( $k, $this->replace_site_url($v) );
			}
		} else {
			if ($this->options['debug'])
