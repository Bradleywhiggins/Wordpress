<div class="front_page_section front_page_section_testimonials<?php
			$cleanskin_scheme = cleanskin_get_theme_option( 'front_page_testimonials_scheme' );
if ( ! cleanskin_is_inherit( $cleanskin_scheme ) ) {
	echo ' scheme_' . esc_attr( $cleanskin_scheme );
}
			echo ' front_page_section_paddings_' . esc_attr( cleanskin_get_theme_option( 'front_page_testimonials_paddings' ) );
?>"
		<?php
		$cleanskin_css      = '';
		$cleanskin_bg_image = cleanskin_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$cleanskin_anchor_icon = cleanskin_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$cleanskin_anchor_text = cleanskin_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( cleanskin_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$cleanskin_css      = '';
			$cleanskin_bg_mask  = cleanskin_get_theme_option( 'front_page_testimonials_bg_mask' );
			$cleanskin_bg_color_type = cleanskin_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $cleanskin_bg_color_type ) {
				$cleanskin_bg_color = cleanskin_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$cleanskin_caption = cleanskin_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $cleanskin_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $cleanskin_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $cleanskin_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$cleanskin_description = cleanskin_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $cleanskin_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $cleanskin_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $cleanskin_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
				...
				if ( ! cleanskin_exists_trx_addons() ) {
					...
					cleanskin_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
				}
			}
			?>
			</div>
